...
**Step 1:** Download the LiDAR data from the following link: <https://opendata.vancouver.ca/explore/dataset/lidar-2022/information/>

THe data is split up into a tiled grid system. You will download one tile covering a section of UBC. Use the "Table" view and search for the following tile and download it: "484000_5455000" by clicking on the .zip link. The zip file is about 2 GB compressed and about 4 GB extracted, so you will need 6 GB of free space on your computer to complete this step. After you have extracted the las file, you can delete the zip file to free up space on your computer.
...
-   Input File: 484000_5455000.las
-   Output LAS Dataset: pacific_spirit_park.lasd
-   Coordinate System: Use the horizontal datum from the dataset specifications
-   Create PRJ for LAS Files: All LAS files
-   Check the "Compute Statistics" box.
-   Surface Constraints can be left blank

```{r, echo=FALSE, fig.align = "center", out.width = "50%"}
knitr::include_graphics("images/05-lasd.png")
```

**Step 3:** Depending on the zoom extent, you may only see the red bounding box of the las file; this is not an error, you just need to zoom in to see the actual points.
...
There is also a more detailed classification system, which includes vegetation, water, noise, buildings etc. Right-click the file, select "Properties", then select "LAS Filter". This menu gives you more control for which points you want to display from the dataset.
...
-   Digital Elevation Model (DEM) - sometimes referred to as a Digital Terrain Model (DTM)
...
Analysis , Tools , and Search for "LAS Dataset to Raster". We will use the "Binning" method to interpolate elevation. This method works by dividing the point cloud into cells (pixels) and assigning a value to each cell based on the heights of all the points in the cell. For example, cell values could be assigned as the average height of all points within a 10x10m pixel.

-   Input LAS Dataset: pacific_spirit_park.lasd
-   Output Raster: DEM
...
**Step 2:** Next, we will create a DSM. We need to ensure that all points labelled as "Noise" are filtered out from the dataset, we will also filter out "Ground" points and just retain points coming from vegetation/infrastructure. Navigate to the LAS Filter menu in the data Properties tab.
...
Once you have correctly filtered the point cloud open the "LAS Dataset to Raster" tool. This time we will use the "Maximum" value as the Cell Assignment. Name the output raster "DSM". All other settings can stay the same as the DEM. Click "Run".
...
##### Screenshot 1: Upload a screenshot of your final CHM. Change the default symbology to show unvegetated areas \< 2 m, medium vegetation \< 30 m, and tall vegetation greater than 30 m. (15 points) {.unnumbered}
...
**Step 5:** Open the "Raster Calculator" tool. We will use this tool to find the pixels in the "TreeTops" raster that match the maximum focal height in the "TreeTops_max". To do this, we will use a True/False conditional statement using the `Con` syntax:
...
**Step 6:** Open the "Raster to Point" tool. The input raster is the raster that you just made. Field is Value, and the output name should be "tree_with_height".
...
On the top ribbon, select "Insert", then "New Map", then "New Local Scene".

**Step 1:** Practice using the On Screen Navigator to manipulate the scene. The first toggle lets you navigate in the normal directions and the second toggle (the person) lets you rotate the scene in 3D.
