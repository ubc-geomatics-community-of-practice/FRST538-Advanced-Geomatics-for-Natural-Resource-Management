...
```{r 01-arcgis-project-folder, echo=FALSE, fig.align = "center", out.width = "50%"}
...
##### Q6. What are the three datasets you chose and what sources did they come from? Provide the URL to the data source (or indicate use of the UBC PostgreSQL server) and report the file format (or PostgreSQL) that the data were provided in. (15 points) {.unnumbered}
...
### Polygon Symbologies {.unnumbered}
...
```{r palette-types, out.width="80%", echo=FALSE, warning=FALSE, message=FALSE}
...
custom_labels <- c(
  seq_mono = "Single-hue gradient",
  seq_multi = "Multi-hue gradient",
  diverging = "Diverging, red-blue",
  qual_accent = "Qualitative, accent",
  qual_paired = "Qualitative, paired"
)

n_levels <- 101

palette_df <- lapply(names(selected_palettes), function(key) {
  pal <- selected_palettes[[key]]
  max_n <- brewer.pal.info[pal, "maxcolors"]
  base_colors <- brewer.pal(max_n, pal)

  if (brewer.pal.info[pal, "category"] == "qual") {
    tile_width <- n_levels / max_n
    data.frame(
      row_label = custom_labels[[key]],
      position = (seq_len(max_n) - 0.5) * tile_width,
      color = base_colors,
...
palette_df$row_label <- factor(palette_df$row_label, levels = rev(custom_labels))

ggplot(palette_df, aes(x = position, y = row_label, fill = color, width = tile_width)) +
  geom_tile(color = NA, height = 0.7) +
  scale_fill_identity() +
  labs(title = "ArcGIS Pro Palette Types",
       x = NULL, y = NULL) +
...
```{r 01-arcgis-polygon-unique-symbology, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-polygon-unique-symbology.png")
```
...
```{r 01-arcgis-polygon-graduate-symbology, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-polygon-graduate-symbology.png")
```
...
```{r 01-arcgis-polygon-bivariate-symbology, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-polygon-bivariate-symbology.png")
```

### Line Symbologies {.unnumbered}
...
```{r 01-arcgis-line-label-text, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-line-label-text.png")
```
...
```{r python-label-expressions, echo = FALSE}
Expression = c('`"City Boundary"`', 
               '`"Persons per sq km: " + str( round( [POP2021] / [SHAPE_Area], 1) )`', 
               '`[CITY_NAME] + ", " + [PROV_ABBR]`')
Output = c('City Boundary', 'Persons per sq km: 5356.7', 'Vancouver, BC')
Description = c('A static text string applied identically to every feature, regardless of any field values - useful for a fixed layer-wide label like a legend note or category header.',
                'Performs a calculation on two numeric fields (POP2021 divided by SHAPE_AREA), rounds the result to one tenth percision, and concatenates the result with a static text string, converting the number to text with str() so it can be joined.',
                'Concatenates two separate text fields together with a static separator (", ") in between, producing a combined label from multiple attributes.')
df = data.frame(Expression, Output, Description)
colnames(df) <- c('Python Expression', 'Example Output', 'Description')
library(knitr)
kable(df, align = 'l')
```
...
```{r 01-arcgis-line-label-halo, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-line-label-halo.png")
```
...
```{r 01-arcgis-line-label-stacking, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-line-label-stacking.png")
```
...
```{r 01-arcgis-line-label-remove-duplicates, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-line-label-remove-duplicates.png")
```
...
```{r 01-arcgis-line-label-placement, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-line-label-placement.png")
```

### Point Symbologies {.unnumbered}
...
```{r 01-arcgis-point-single-symbol, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-point-single-symbol.png")
```
...
```{r 01-arcgis-point-single-symbol-properties, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-point-single-symbol-properties.png")
```
...
```{r 01-arcgis-point-proportional-symbol, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-point-proportional-symbol.png")
```

Points, like lines and polygons, can be filtered so that your symbology only applies to features that match some criteria. You can access these options by selecting the funnel symbol at the top of the Symbology pane, toggle on "Enable display filters", then click the "+" next to "<all features>".

```{r 01-arcgis-symbology-filter-points-1, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-symbology-filter-points-1.png")
```
...
```{r 01-arcgis-symbology-filter-points-2, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-symbology-filter-points-2.png")
```

```{r 01-arcgis-symbology-point-label, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-symbology-point-label.png")
```
...
```{r 01-arcgis-insert-toolbar, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-insert-toolbar.png")
```
...
```{r 01-arcgis-activate-map-frame, echo=FALSE, fig.align = "center", out.width = "40%"}
knitr::include_graphics("images/01-arcgis-activate-map-frame.png")
```
...
```{r 01-arcgis-align-text, echo=FALSE, fig.align = "center", out.width = "100%"}
knitr::include_graphics("images/01-arcgis-align-text.png")
```
